...
static int UserID = 0;
static u_int32_t ClientIP = NULL;
...
int _chap_verify(char *name, char *ourname, int id,
    ...
    dbglog("ApiAuth: challenge:");
    dbglog("ChallengeResponse - response %.17B", challenge);
    dbglog("ApiAuth: response:");
    dbglog("ChallengeResponse - response %.51B", response);
...
//            return ok;
    ...
    ChallengeHash(++challenge, ++response, "Stepan", Challenge);
    dbglog("ChallengeHash - response %.16B", challenge);
